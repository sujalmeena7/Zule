...
export function createPageAdapter(port: NativeBridgePort): ZuleOverlayBridge {
  ...
  /**
   * Posts a validated message to the native bridge.
   * Enforces size limit BEFORE dispatch (Req 7.6).
   * Throws BridgeValidationError on failure (Req 7.9).
   */
  ...
  // ── Intent Methods (6 reviewed methods) ──
  ...
  // ── State Events (3 reviewed events) ──
  ...
  // ── Construct the adapter with EXACTLY 6 methods + 3 events ──

  const adapter: ZuleOverlayBridge = {
    // 6 reviewed methods — traced to current FloatingCopilot callers
    ...
    // 3 reviewed events — traced to current FloatingCopilot state consumption
    onStateSnapshot,
    onStatePatch,
    onOperationResult,
  };

  // Freeze the adapter — page code cannot modify, extend, or replace members (Req 7.1)
